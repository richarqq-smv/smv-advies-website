...
> **Belangrijke correctie (websitetransformatie)**: dit document (en fase 18/19 van `SEO-ROADMAP.md`) ging er destijds van uit dat de 3 cases echte, bestaande klantresultaten waren ("carried over from the current live site"). De eigenaar heeft dit expliciet gecorrigeerd: het bedrijf is in opstartfase (KvK-registratie nog niet afgerond), en de cases waren fictieve placeholder-content die nooit als echte klantresultaten hadden mogen worden gepresenteerd. De cases (`cases.js`, `casesDetailed.js`, `CaseCard.jsx`, `CaseDetailCard.jsx`, `CasesSection.jsx`) zijn daarom volledig verwijderd; `/cases` legt nu uit hoe een adviestraject werkt, zonder te claimen waaróm er nog geen praktijkcases beschikbaar zijn voor publicatie. Bevindingen hieronder die uitgaan van "echte cases" zijn historisch en niet langer van toepassing — zie `WEBSITE_STRATEGY.md` en `CONTENT_NEEDED.md` voor de actuele aanpak.
