...
| 19 | Vervolgaudit commercieel/conversie (herbevestiging fase 18 + nieuwe bevinding) | `/pakketten` (bestaande pagina gewijzigd, geen nieuwe URL) | 2026-09-04 (sessiedatum) | n.v.t. | `b22051c` | Opdracht kwam binnen met een context die fase 18 niet vermeldde ("fase 15, 16 en 17 zijn afgerond"); `git log` bevestigde dat fase 18 al volledig was uitgevoerd en live stond. In plaats van dezelfde audit te herhalen is gecontroleerd of er sindsdien iets is gewijzigd (KvK nog steeds `null`, alle 6 image-placeholders nog aanwezig — geen eigenaarsinput bijgekomen) en is een pagina onderzocht die fase 18 niet had bekeken: `/pakketten`. Daar bleek de hero dakisolatie/warmtepomp/zonnepanelen/LED-verlichting/EIA/ISDE bij naam te noemen zonder te linken (zelfde, al meermaals opgeloste patroon als fase 17/18), én had de pagina geen enkele link naar `/cases` — een concreet gat in het door de opdracht gevraagde BLOG → DIENST → CASE → CTA-pad. Beide opgelost met inline links, geen andere wijziging. |
...
## Commerciële/conversie-audit — fase 19 (vervolg op fase 18)

**Context-mismatch bij binnenkomst van de opdracht**: de fase-19-opdracht vroeg om exact dezelfde commerciële/conversie- en vertrouwensaudit als fase 18 (homepage, energie-indicatie, contact, cases, sitestructuur, trust/autoriteit), maar de meegegeven context vermeldde alleen "fase 15, 16 en 17 zijn afgerond" — fase 18 ontbrak. `git log` bevestigde dat `HEAD` bij aanvang al `5aedd11` was (fase 18's fix + documentatie), dus fase 18 was al volledig uitgevoerd en live. In plaats van de identieke audit te herhalen (wat geen waarde toevoegt) is gecontroleerd of er sindsdien iets is gewijzigd waardoor eerdere bevindingen niet meer kloppen, en is vervolgens gezocht naar een pagina die fase 18 niet had onderzocht.

**Herbevestigd, ongewijzigd sinds fase 18**:
- `src/data/company.js`: `kvk` is nog steeds `null`.
- Alle 6 `ImagePlaceholder`-labels ("... - nog te plaatsen") in `cases.js`/`casesDetailed.js` en de homepage-hero zijn nog steeds aanwezig — geen nieuwe fotografie aangeleverd.
- De energie-indicatietool vraagt nog steeds volledige contactgegevens in stap 4 vóór enig resultaat — bewust niet aangepast, zoals expliciet gevraagd (businessbeslissing, geen technisch mankement).

**Nieuwe bevinding — `/pakketten` (niet onderzocht in fase 18)**: dit is de belangrijkste DIENST-pagina van de site (`PageHero` in `src/pages/Pakketten.jsx`), en noemde dakisolatie, een warmtepomp, zonnepanelen, LED-verlichting en de EIA/ISDE-subsidies in platte tekst — exact hetzelfde, al meermaals bevestigde patroon dat in fase 17 (netcongestie/warmtepomp/LED) en fase 18 (energie-indicatie) is opgelost. Daarnaast bevatte de pagina, ondanks een aparte "Het verschil in één oogopslag"-sectie en een `DecisionCta`, geen enkele link naar `/cases` — de opdracht vroeg expliciet om het pad BLOG → DIENST → CASE → CTA in kaart te brengen, en dit was daarin de ontbrekende schakel (DIENST had geen uitgaande link naar CASE).

**Waarom dit de hoogste-waarde, wél zelfstandig uitvoerbare verbetering was**: het patroon (onderwerp bij naam genoemd, niet gelinkt) is in twee eerdere fases al erkend als een concrete, meetbare verbetering (houdt bezoekers die op zoek zijn naar een specifieke maatregel op de site, i.p.v. dat ze een losstaande zin zonder vervolgstap lezen) en vereist geen eigenaarsinput, geen nieuwe content en geen wijziging aan gedeelde componenten (`PageHero`/`SectionHeading` renderen hun `description`-prop al als vrije JSX, geverifieerd door beide bestanden te lezen vóór de wijziging).

**Doorgevoerde verbetering (fase 19)**:
1. `src/pages/Pakketten.jsx`: de hero-tekst linkt nu naar `dakisolatie-voor-uw-bedrijfspand`, `warmtepomp-in-het-mkb`, `zonnepanelen-op-uw-bedrijfspand`, `led-verlichting-snelste-stap` en `eia-isde-sde-subsidies`.
2. `src/pages/Pakketten.jsx`: de "Het verschil in één oogopslag"-sectie eindigt nu met een link naar `/cases` ("Bekijk onze cases"), zodat de DIENST-pagina nu doorverwijst naar het bewijs (CASE).

**Overige onderdelen van de gevraagde audit (homepage, energie-indicatie, contact, cases, trust/autoriteit, lokale SEO)**: opnieuw doorgelopen, geen nieuwe bevindingen ten opzichte van fase 18 — de eerder geïmplementeerde fixes staan nog live en ongewijzigd, en er is geen nieuwe eigenaarsinput beschikbaar gekomen die een van de destijds als "vereist eigenaarsinput" gemarkeerde punten alsnog uitvoerbaar maakt.
...
**BLOG → DIENST → CASE → CTA (in kaart gebracht in fase 19)**: blogartikelen linken naar `/pakketten` (via bestaande artikel-links) en naar `/energie-indicatie`/`/contact` (CTA); `/pakketten` linkt sinds fase 19 terug naar de vijf meest relevante blogartikelen én door naar `/cases` (CASE); `/cases` linkt sinds fase 18 per case door naar de bijbehorende blogartikelen (`relatedArticles`) en sluit af met dezelfde `ClosingCta` (energie-indicatie + contact) als de rest van de site. Het pad is daarmee in beide richtingen doorlopend: BLOG ⇄ DIENST ⇄ CASE ⇄ CTA.
...
| 5 | ~~Commerciële/conversie-SEO~~ | ~~De paden content → `/energie-indicatie` → `/contact` optimaliseren~~ | Meer content zonder conversieverbetering levert minder waarde op dan gerichte funnel-optimalisatie | — | **Uitgevoerd in fase 18, vervolgd in fase 19** (interne links, incl. BLOG↔DIENST↔CASE↔CTA-pad); resterende bevindingen vereisen eigenaarsinput of een businessbeslissing (zie "Commerciële/conversie-audit — fase 18" en "— fase 19") | Uitgevoerd op 2026-09-01, vervolgd op 2026-09-04 |
